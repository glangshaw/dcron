...
-c dir
:	directory of per-user crontabs (defaults to /var/lib/cron/crontabs)

-t dir
:	directory of timestamps for @freq and FREQ=... jobs
	(defaults to /var/lib/cron/cronstamps)
...
in the per-user crontabs directory (usually /var/lib/cron/crontabs). **crontab**
...
files from its timestamp directory (usually /var/lib/cron/cronstamps). Normally
